...
async def handle_antiflood_callbacks(query, data: str, cid: int, user, user_states):
    ...
    if data.startswith("cfg_view_flood_"):
        ...
        text = f"Send now the duration of the chosen punishment ({ptype})\n\n<b>Minimum:</b> 30 seconds\n<b>Maximum:</b> 365 days\n\n<b>Example:</b> <code>30 minutes</code>"
        kb = [[create_btn("0️⃣ Remove duration", callback_data=f"flrem_dur_{cid}")], [create_btn("❌ Cancel", callback_data=f"cfg_view_flood_{cid}", style="danger")]]
