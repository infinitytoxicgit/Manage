  ...
  async with httpx.AsyncClient(timeout=10.0) as client:
    try:
      ...
      if resp.status_code == 200:
        data = resp.json()
        translated = "".join(
            [piece[0] for piece in data[0] if piece and piece[0]]
        )
        return translated
...
async def handle_regulations_callbacks(
  ...
  if data.startswith("trcancel_"):
    ...
    if user.id != target_uid and not await is_user_admin(cid, user.id, context):
      await query.answer(
          "Sirf command send karne wala cancel kar sakta hai!", show_alert=True
      )
      return
  ...
  if data.startswith("trset_"):
    ...
    if user.id != target_uid and not await is_user_admin(cid, user.id, context):
      await query.answer(
          "Sirf command sender language choose kar sakta hai!", show_alert=True
      )
      return
    ...
    if not orig_text:
      await query.answer(
          "Translation session expire ho gaya. Kripya dobara /translate"
          " karein.",
          show_alert=True,
      )
      try:
    ...
    translated_result = await perform_google_translate_async(
        orig_text, lang_code
    )
